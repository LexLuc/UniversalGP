...
def chunking(_):
    ...
    """
    num_inducing = 100
#    X = np.load('path/to/x_train.npz')
#    Y = np.load('path/to/y_train.npz')

    X = np.linspace(0, 5, num=300, endpoint=False, dtype=np.float32)[:, np.newaxis]
...
#            inducing_inputs=_init_z(x_train, num_inducing),
            inducing_inputs=x_train[::num_train//num_inducing],
